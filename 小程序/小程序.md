...
> 1.网络请求在onLoad生命周期函数中
>
> 2. sucess中使用箭头函数，因为要获取data中的数据进行设置

### 网络请求域名配置！！！

每个微信小程序需要事先**设置通讯域名**，小程序**只可以跟指定的域名进行网络通信**。 

+ **小程序登录后台 – 开发管理 – 开发设置 – 服务器域名；*（才能向服务器发送请求）**

> 在对域名进行配置后，会对域名进行验证，有没有备案是否合法

 **服务器域名请在 「小程序后台 - 开发 - 开发设置 - 服务器域名」 中进行配置，配置时需要注意：**

+ 域名只支持 **https** (wx.request、wx.uploadFile、wx.downloadFile) 和 wss (wx.connectSocket) 协议； 
+ 域名不能使用 **IP 地址（小程序的局域网 IP 除外）或 localhost；** 
+ 可以**配置端口**，如 https://myserver.com:8080，但是配置后只能向 https://myserver.com:8080 发起请求。如果向 https://myserver.com、https://myserver.com:9091 等 URL 请求则会失败。
...
+ 不支持配置父域名，使用子域名。
+ ![image-20240302144157794](https://raw.githubusercontent.com/krystalkrystaljj/myimg/main/image-20240302144157794.png)
...
只需要传入options，可以不写success和fail函数，通过promise拿到请求成功和失败后的值



![image-20240302160128272](https://raw.githubusercontent.com/krystalkrystaljj/myimg/main/image-20240302160128272.png)



![image-20240302160358088](https://raw.githubusercontent.com/krystalkrystaljj/myimg/main/image-20240302160358088.png)

![image-20240302160634705](https://raw.githubusercontent.com/krystalkrystaljj/myimg/main/image-20240302160634705.png)

2 类 -> 创建实例

+ 根据创建出来的实例每个baseURL不一样
+ 可以封装get和post方法
...
+ 监听用户点击**页面内转发按钮（button 组件 open-type="share"）**或**右上角菜单“转发”按钮的行为**，并自定义转发内容。 
